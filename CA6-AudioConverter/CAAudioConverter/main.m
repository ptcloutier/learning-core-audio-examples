...
//  CAAudioConverter
//
//  Created by Perrin Cloutier on 4/18/18.
...
/* Audio format conversion from Learning Core Audio. ARC Disabled. */

#define kInputFileLocation CFSTR("/Users/perrincloutier/Music/ya.m4a")
...
OSStatus MyAudioConverterCallback(AudioConverterRef inAudioConverter,
    ...
    (MyAudioConverterSettings *)inUserData;
    ioData->mBuffers[0].mData = NULL;
    ioData->mBuffers[0].mDataByteSize = 0;
    // If there are not enough packets to satisfy request,
    // then read what's left
    if (audioConverterSettings->inputFilePacketIndex + *ioDataPacketCount >
        audioConverterSettings->inputFilePacketCount)
        *ioDataPacketCount = audioConverterSettings->inputFilePacketCount -
    ...
    audioConverterSettings->sourceBuffer = (void *)calloc(1, *ioDataPacketCount *
...
void Convert(MyAudioConverterSettings *mySettings) {
    ...
    if (sizePerPacket == 0) {
        ...
        mySettings->inputFilePacketDescriptions = (AudioStreamPacketDescription*)
        malloc(sizeof(AudioStreamPacketDescription) * packetsPerBuffer);
    } else {
        packetsPerBuffer = outputBufferSize / sizePerPacket;
    }
    ...
    while(1) {
        ...
        convertedData.mBuffers[0].mNumberChannels =
        mySettings->inputFormat.mChannelsPerFrame;
